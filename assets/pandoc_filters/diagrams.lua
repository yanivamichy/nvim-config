...
local function cachedir()
  ...
  if not cache_home or cache_home == '' then
    local user_home = system.os == 'windows' and os.getenv 'USERPROFILE' or os.getenv 'HOME'
  ...
  compile = function(self, code)
    ...
    return with_temporary_directory('diagram', function(tmpdir)
      return with_working_directory(tmpdir, function()
        ...
        pipe(self.execpath or 'mmdc', { '--pdfFit', '--input', infile, '--output', outfile }, '')
...
--           ['additional-packages'] = {
--             pandoc.RawInline('latex', stringify(user_opts['additional-packages'] or '')),
--           },
--         }
--         local tex_code =
--           pandoc.write(pandoc.Pandoc({ pandoc.RawBlock('latex', src) }, meta), 'latex', { template = tikz_template })
...
#set page(width: auto, height: auto, margin: .5cm)
    ...
    return with_temporary_directory('diagram', function(tmpdir)
      return with_working_directory(tmpdir, function()
        ...
        pipe(execpath or 'typst', { 'compile', '-f', format, '-', outfile }, typst_code)
        return read_file(outfile), mime_type
      end)
    end)
  end,
...
local function format_options(name)
  ...
  local preferred_mime_types = is_office_format and pandoc.List { 'image/png', 'application/pdf' }
  ...
  return {
    ...
    best_mime_type = function(self, supported_mime_types, requested)
      return self.preferred_mime_types:find_if(function(preferred)
        return supported_mime_types[preferred]
          and (
            not requested
            or (pandoc.utils.type(requested) == 'List' and requested:includes(preferred))
            or (pandoc.utils.type(requested) == 'table' and requested[preferred])
            -- Assume string, Inlines, and Blocks values specify the only
            -- acceptable MIME type.
            or stringify(requested) == preferred
          )
      end)
    end,
  }
...
local function get_engine(name, engopts, format)
  local engine = default_engines[name] or select(2, pcall(require, stringify(engopts.package)))
  ...
  local mime_type = format:best_mime_type(engine.mime_types, engopts['mime-type'] or engopts['mime-types'])
...
local function code_to_figure(conf)
  return function(block)
    ...
    local run_pdf2svg = engine.mime_type == 'application/pdf' and conf.format.pdf2svg

    -- Try to retrieve the image data from the cache.
    local imgdata, imgtype
    if conf.cache then
      imgdata, imgtype = get_cached_image(pandoc.sha1(block.text), run_pdf2svg and 'image/svg+xml' or engine.mime_type)
    end

    if not imgdata or not imgtype then
      -- No cached image; call the converter
      local success
      success, imgdata, imgtype = pcall(engine.compile, engine, block.text, dgr_opt.opt)
    ...
    -- image content.
    local basename, _extension = pandoc.path.split_extension(dgr_opt.filename or pandoc.sha1(imgdata))
